...
<aside class="fixed inset-y-0 left-0 z-50 w-64 bg-[#0F0F0F] border-r border-[#222] transform transition-transform duration-300 md:relative md:translate-x-0 flex flex-col font-['Inter']"
    ...
    <!-- New Chat -->
    <div class="px-3 pb-6">
        <button @click="createNewChat()" class="w-full flex items-center px-4 py-2.5 rounded-lg border border-[#333] hover:border-[#444] hover:bg-[#1A1A1A] transition-all group group bg-transparent text-white text-sm font-medium">
                ...
                <div class="block px-3 py-2 rounded-lg text-sm transition-all truncate mb-1" 
                    ...
                    <div class="flex items-center justify-between">
                        <a @click="loadConversation(convo.id)" class="flex items-center flex-1 min-w-0 cursor-pointer">
                            <svg class="w-3.5 h-3.5 mr-3 opacity-70 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg>
                            <span class="truncate" x-text="convo.title"></span>
                        </a>
                        <button 
                            @click="deleteConversation(convo.id, convo.title)"
